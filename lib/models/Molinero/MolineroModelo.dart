...
class MolinoModel {
  ...
  MolinoModel(
      this.id_account, this.id_sucursal, this.id_negocio, this.id_admin, this.token) {
...
//Obtener los datos de estadistica semanal
  Future<Map<String, dynamic>> getEstadisticas() async {
    final url = Uri.parse(ApiConfig.backendUrl + '/molinero/registros/semana/$id_account');
...
//obtener los registros de maiz que no han sido pesados
  Future<List<Map<String, dynamic>>> getMaizSinPesar() async {
    final url = Uri.parse(ApiConfig.backendUrl + '/molinero/cocer/maiz/sin_peso/$id_account');
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Error al obtener los maíces sin pesar: ${response.body}');
    }
  }
